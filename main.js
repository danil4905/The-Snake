...
function loop() {
    ...
    ctx.fillStyle = '#74b572';
    for (let i = 0; i < trail.length; i++) {
        ctx.fillStyle = trail[i].color || '#74b572';
...
function changeDirection(evt) {
     ...
     */
    if (evt.keyCode == 37 && !(xv > 0)) // left 
    { 
        xv = -speed; yv = 0; 
    }
    if (evt.keyCode == 38 && !(yv > 0)) // top 
    { 
        xv = 0; yv = -speed; 
    }
    if (evt.keyCode == 39 && !(xv < 0)) // right 
    { 
        xv = speed; yv = 0; 
    }
